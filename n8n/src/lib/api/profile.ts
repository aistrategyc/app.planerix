...
export const getOrgMembers = async (orgId: string): Promise<TeamMember[]> => {
  const { data } = await api.get(`orgs/${orgId}/memberships/`, { params: { expand: "user" } })
  const items: Array<Membership & { user?: UserProfile }> = data?.items ?? data ?? []
  return items
    .map((membership) => {
      ...
      return {
        ...
        is_active: (user as UserProfile & { is_active?: boolean }).is_active,
        last_login_at: (user as UserProfile & { last_login_at?: string | null }).last_login_at ?? null,
      }
    })
