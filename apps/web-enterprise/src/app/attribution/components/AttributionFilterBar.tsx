...
export function AttributionFilterBar({
  ...
  const activeFilters = [
    value.dateRange?.from && value.dateRange?.to
      ...
      : null,
    value.compareMode !== "none" ? `Сравнение: ${compareOptions.find((opt) => opt.value === value.compareMode)?.label ?? value.compareMode}` : null,
    value.cityId !== "all" ? `Город: ${selectedCity?.city_name ?? value.cityId}` : null,
    value.platform !== "all" ? `Платформа: ${value.platform}` : null,
    value.channel !== "all" ? `Канал: ${value.channel}` : null,
    value.device !== "all" ? `Устройство: ${value.device}` : null,
    value.conversionType !== "all" ? `Конверсии: ${value.conversionType}` : null,
      ...
      <Card className="border-muted/70 shadow-sm glass-panel">
        <CardContent className="flex flex-col gap-3 py-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {activeFilters.length === 0 ? (
                <span className="text-xs text-muted-foreground">Нет активных фильтров</span>
              ) : (
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            />
            <Select value={value.platform} onValueChange={(platform) => onChange({ platform })}>
            ...
            <Select value={value.channel} onValueChange={(channel) => onChange({ channel })}>
              ...
              <SelectContent>
                ...
                <SelectItem value="paid_gads">Paid GAds</SelectItem>
            ...
            <Select value={value.conversionType} onValueChange={(conversionType) => onChange({ conversionType })}>
              <SelectTrigger className="h-8 w-[170px]">
                <SelectValue placeholder="Тип конверсии" />
              </SelectTrigger>
              <SelectContent>
...
function CitySelect({ value, label, cities, onValueChange }: CitySelectProps) {
  ...
  const cityLabel = value === "all" ? "Все города" : label ?? "Город"
