...
export const useCalendar = (startDate?: Date, endDate?: Date) => {
  ...
  const fetchEvents = useCallback(async () => {
    try {
      ...
      const fetchedEvents = await CalendarAPI.getEvents(params)
      setEvents(fetchedEvents)
    } catch (err: unknown) {
      const error = err as { response?: { data?: { detail?: string } }; message?: string }
      const errorMessage = error.response?.data?.detail || error.message || 'Failed to fetch calendar events'
  ...
  const bootstrapRituals = useCallback(async (): Promise<boolean> => {
    try {
      const result = await CalendarAPI.bootstrapRituals()
      await fetchEvents()
      toast({
        title: 'Rituals Updated',
        description: `OKR reviews: +${result.okr_reviews_created}, Milestones: +${result.project_milestones_created}`,
      })
      return true
    } catch (err: unknown) {
      const error = err as { response?: { data?: { detail?: string } }; message?: string }
      const errorMessage = error.response?.data?.detail || error.message || 'Failed to create rituals'
      toast({
        title: 'Error',
        description: errorMessage,
        variant: 'destructive',
...
export const useCalendarStats = () => {
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      setError(error.message || 'Failed to fetch calendar statistics')
    } finally {
      setLoading(false)
    }
  }, [])
...
export const useUpcomingDeadlines = (days: number = 7) => {
  ...
  const fetchDeadlines = useCallback(async () => {
    try {
      ...
      const error = err as { message?: string }
      setError(error.message || 'Failed to fetch upcoming deadlines')
    } finally {
      setLoading(false)
    }
  }, [days])
...
export const useOverdueEvents = () => {
  ...
  const fetchOverdueEvents = useCallback(async () => {
    try {
      ...
      const error = err as { message?: string }
      setError(error.message || 'Failed to fetch overdue events')
    } finally {
      setLoading(false)
    }
  }, [])
...
export const useEventFilters = (events: CalendarEvent[]) => {
  ...
  }, [events, searchQuery, selectedTypes, selectedStatuses])
