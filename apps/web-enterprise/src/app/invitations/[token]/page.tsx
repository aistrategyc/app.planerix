...
export default function InvitationAcceptPage() {
  ...
  const preflight = useCallback(async () => {
    ...
    try {
      ...
      setError(error?.response?.data?.detail || "Приглашение не найдено или истекло");
      setData(null);
    } finally {
      setLoading(false);
    }
  ...
  const handleAccept = useCallback(async () => {
    ...
    try {
      ...
      const error = e as { response?: { data?: { detail?: string } } }
      alert(error?.response?.data?.detail || "Не удалось принять приглашение");
    } finally {
      setSubmitting(false);
    }
  ...
  const handleReject = useCallback(async () => {
    ...
    try {
      ...
      const error = e as { response?: { data?: { detail?: string } } }
      alert(error?.response?.data?.detail || "Не удалось отклонить приглашение");
    } finally {
      setSubmitting(false);
    }
