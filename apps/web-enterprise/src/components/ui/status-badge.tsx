...
const statusBadgeVariants = cva(
  "inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 shadow-sm",
  {
    variants: {
      variant: {
        default: "border-transparent bg-gradient-to-r from-indigo-500 via-blue-500 to-cyan-400 text-white",
...
export function PriorityBadge({ priority }: { priority: "low" | "medium" | "high" | "critical" | "urgent" }) {
  const variantMap = {
  ...
  return (
    <StatusBadge 
      ...
      className={cn(
        "font-medium",
        (priority === "critical" || priority === "urgent") && "animate-pulse"
      )}
    >
