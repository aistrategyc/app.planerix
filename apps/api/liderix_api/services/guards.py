...
# Роли и права
# -----------------------------
Role = Literal["owner", "admin", "bu_manager", "hod", "team_lead", "pmo", "manager", "member", "guest"]
# Базовые права по ролям (org-wide).
# Для объектных операций используйте check_perm/require_perm c ResponsibilityScope.
ROLE_PERMS = {
    "owner": {"org:read", "org:write", "org:invite", "member:read", "member:manage", "dept:write"},
    "admin": {"org:read", "org:write", "org:invite", "member:read", "member:manage", "dept:write"},
    "bu_manager": {"org:read", "org:write", "member:read", "member:manage", "dept:write"},
    "hod": {"org:read", "member:read", "dept:write"},
    "team_lead": {"org:read", "member:read", "dept:write"},
    "pmo": {"org:read", "member:read"},
    "manager": {"org:read", "member:read", "dept:write"},
...
async def _get_org(session: AsyncSession, org_id: UUID) -> Organization:
    org = await session.get(Organization, org_id, options=[noload("*")])
