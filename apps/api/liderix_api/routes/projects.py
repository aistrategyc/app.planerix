...
from liderix_api.enums import ProjectStatus, ProjectPriority, MembershipRole, Permission, membership_has_permission, TaskStatus

router = APIRouter(tags=["Projects"])
logger = logging.getLogger(__name__)
ORG_WIDE_PROJECT_ROLES = {MembershipRole.OWNER, MembershipRole.ADMIN, MembershipRole.BU_MANAGER, MembershipRole.PMO}

# Normalize status inputs across schema/enums
def _normalize_status(value: object, fallback: str) -> str:
    ...
    allowed = {status.value for status in ProjectStatus}
...
def _normalize_priority(value: object, fallback: str) -> str:
    ...
    raw = str(value).strip().lower()
    aliases = {
        "critical": ProjectPriority.URGENT.value,
    }
    normalized = aliases.get(raw, raw)
    allowed = {priority.value for priority in ProjectPriority}
    return normalized if normalized in allowed else fallback
...
async def _get_project_health(session: AsyncSession, project_id: UUID, org_id: Optional[UUID]) -> ProjectHealthResponse:
    """Calculate project health score (delivery + KPI + risk)."""
    now = now_utc()
    ...
    task_distribution = {status: int(count or 0) for status, count in task_stats}

    total_tasks = sum(task_distribution.values())
    completed_tasks = task_distribution.get(TaskStatus.DONE, 0) + task_distribution.get(TaskStatus.CANCELLED, 0)
    blocked_tasks = task_distribution.get(TaskStatus.BLOCKED, 0)
    in_review_tasks = task_distribution.get(TaskStatus.IN_REVIEW, 0)
    ...
    overdue_tasks = await session.scalar(
        select(func.count(Task.id)).where(
            and_(
                Task.project_id == project_id,
                Task.deleted_at.is_(None),
                Task.status.in_(TaskStatus.get_active_statuses()),
    ...
    overdue_rate = (overdue_tasks / total_tasks) if total_tasks else 0.0
    blocked_rate = (blocked_tasks / total_tasks) if total_tasks else 0.0

    delivery_score = completion_rate - (overdue_rate * 40) - (blocked_rate * 30)
    delivery_score = max(0.0, min(100.0, delivery_score))

    kpi_rows = []
    if org_id:
        kpi_rows = (await session.execute(
            select(KPIIndicator.status, KPIIndicator.current_value, KPIIndicator.target_value)
            .where(
                and_(
                    KPIIndicator.project_id == project_id,
                    KPIIndicator.org_id == org_id,
                    KPIIndicator.deleted_at.is_(None),
                )
            )
        )).all()
    ...
    if kpi_rows:
        ...
        for status_value, current_value, target_value in kpi_rows:
            if status_value == "on_track":
                ...
                kpi_critical += 1
            if target_value:
                progress_values.append((current_value or 0.0) / target_value * 100)
        if progress_values:
            kpi_score = sum(progress_values) / len(progress_values)

    active_assignees = await session.scalar(
    ...
    overload_rows = await session.execute(
    ...
    overloaded_members = sum(1 for _, count in overload_rows if (count or 0) > 7)
    overload_ratio = (overloaded_members / active_assignees) if active_assignees else 0.0

    risk_score = min(100.0, overdue_rate * 100 + blocked_rate * 80 + overload_ratio * 50)

    if delivery_score >= 70 and risk_score < 40 and (kpi_score is None or kpi_score >= 70):
        health_status = "green"
    elif delivery_score >= 40 and risk_score < 70:
...
async def _ensure_objective(
    ...
    if not objective:
        problem(400, "urn:problem:invalid-objective", "Invalid Objective", "Objective not found in organization")
...
async def list_projects(
    ...
    # Build base query
    query = select(Project).options(lazyload("*"))
    ...
    memberships = (
        await session.execute(
            select(Membership.org_id, Membership.role).where(
                and_(
    ...
    access_filters.append(Project.owner_id == current_user.id)
    ...
    # Org-wide privileged roles can view all projects in org
    org_wide_org_ids = [
        org_id for org_id, role in memberships
        if role in ORG_WIDE_PROJECT_ROLES and membership_has_permission(role, Permission.PROJECT_VIEW)
    ]
    if org_wide_org_ids:
        access_filters.append(Project.org_id.in_(org_wide_org_ids))
    ...
    if priority:
        filters.append(Project.priority == _normalize_priority(priority, ProjectPriority.MEDIUM.value))
...
@router.post("", response_model=ProjectDetailResponse, status_code=status.HTTP_201_CREATED, include_in_schema=False)
...
async def create_project(
    ...
    member_users = []
    if data.member_ids:
        member_users = await _validate_project_members(session, org_id, data.member_ids)

    await _ensure_objective(session, org_id, data.objective_id)
    ...
    project = Project(
        ...
        org_id=org_id,
        status=_normalize_status(data.status, ProjectStatus.DRAFT.value),
        priority=_normalize_priority(data.priority, ProjectPriority.MEDIUM.value),
...
async def update_project(
    ...
    if "status" in payload:
        payload["status"] = _normalize_status(payload["status"], ProjectStatus.DRAFT.value)
        if payload["status"] == ProjectStatus.COMPLETED.value and not project.completed_at:
            project.completed_at = now_utc()
        elif payload["status"] != ProjectStatus.COMPLETED.value:
            project.completed_at = None

    if "priority" in payload:
        payload["priority"] = _normalize_priority(payload["priority"], ProjectPriority.MEDIUM.value)

    if "objective_id" in payload:
        await _ensure_objective(session, project.org_id, payload.get("objective_id"))
...
async def remove_project_member(
    ...
    if project.owner_id and user_id == project.owner_id:
        problem(
            400,
            "urn:problem:cannot-remove-owner",
            "Cannot Remove Owner",
...
async def get_project_health(
    ...
    project = await _get_project_with_access(session, project_id, current_user, "read")
    result = await _get_project_health(session, project_id, project.org_id)

    await AuditLogger.log_event(
        session, current_user.id, "project.health", True,
        request.client.host if request.client else "unknown",
        request.headers.get("user-agent", "unknown"),
        {"project_id": str(project_id)},
    )
...
async def update_project_status(
    ...
    new_status_value = _normalize_status(new_status, ProjectStatus.DRAFT.value)
    ...
    if new_status_value == ProjectStatus.COMPLETED.value and not project.completed_at:
        project.completed_at = now_utc()
    elif new_status_value != ProjectStatus.COMPLETED.value:
        project.completed_at = None
