...
async def upload_user_avatar(
    ...
    try:
        # Handle file upload and build public URL
        avatar_path = await handle_avatar_upload(file, current_user.id)
        base_url = str(request.base_url).rstrip("/") if request else ""
        avatar_url = avatar_path if avatar_path.startswith("http") else f"{base_url}{avatar_path}"
