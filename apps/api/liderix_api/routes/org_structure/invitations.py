...
async def create_invitation(
    ...
    """
    await require_perm(ctx, "org:invite")
    ...
    inviter_name = (
        (inviter.full_name or "").strip()
        or (inviter.username or "").strip()
        or (inviter.email if inviter else None)
        or "A teammate"
    )
    invite_url = f"{settings.FRONTEND_URL.rstrip('/')}/invitations/{inv.token}"
    subject = f"You've been invited to join {org.name} on Planerix"
    text_body = (
        f"{inviter_name} invited you to join {org.name} as {inv.role}.\n"
        f"Accept your invitation: {invite_url}\n\n"
        "If you did not expect this invite, you can ignore this email."
    )
    html_body = (
        f"<p>{inviter_name} invited you to join <b>{org.name}</b> as <b>{inv.role}</b>.</p>"
