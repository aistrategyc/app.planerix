...
async def check_slug_availability(
    ...
    exists = await session.scalar(
        select(func.count(Organization.id)).where(
            and_(Organization.slug == normalized, Organization.deleted_at.is_(None))
        )
    )
    return {"available": (exists or 0) == 0}
...
async def update_organization(
    ...
    """
    await require_perm(ctx, "org:write")
...
async def delete_organization(
    ...
    """
    await require_perm(ctx, "org:delete")
