...
# ----------------- list -----------------
@router.get("", response_model=DepartmentListResponse, include_in_schema=False)
...
async def get_department_tree(
    ...
    return tree
# ----------------- create -----------------
@router.post("", response_model=DepartmentRead, status_code=status.HTTP_201_CREATED, include_in_schema=False)
...
async def create_department(
    ...
    await require_perm(ctx, "dept:write")
    await _validate_org_exists(session, org_id)
    ...
    dept = Department(
        ...
        policy=data.policy.model_dump() if data.policy else {},
        created_at=now_utc(),
        updated_at=now_utc(),
    )
    session.add(dept)
