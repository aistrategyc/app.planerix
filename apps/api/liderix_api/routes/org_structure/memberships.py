...
ROLE_TRANSITIONS = {role: list(ROLE_HIERARCHY.keys()) for role in ROLE_HIERARCHY}

# Lightweight invitation sender
async def send_invitation_email(
    *,
    ...
    from liderix_api.services.notifications import send_email_notification
    from liderix_api.config.settings import settings

    subject = f"You've been invited to join {org_name} on Planerix"
    link = invite_url or settings.FRONTEND_URL.rstrip("/")
    text_body = (
    ...
    html_body = (
        f"<p>{inviter_name} invited you to join <b>{org_name}</b> as <b>{role}</b>.</p>"
...
# ----------------- list -----------------
@router.get("", response_model=MembershipListResponse, include_in_schema=False)
...
async def list_memberships(
    ...
    await require_perm(ctx, "member:read")
...
async def create_membership(
    ...
    await require_perm(ctx, "member:manage")
...
async def bulk_invite_by_email(
    ...
    for i, item in enumerate(data.memberships):
        try:
            ...
            invite_url = f"{settings.FRONTEND_URL.rstrip('/')}/invitations/{invitation.token}"
            background.add_task(
                send_invitation_email,
                email=item.email,
                org_name=org.name,
